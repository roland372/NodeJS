...
// define how many items should be displayed per page
const ITEMS_PER_PAGE = 2;

exports.getProducts = (req, res, next) => {
	// get information on what page we are
	// + will convert a page to a number, instead of string
	// || use 1 if previous value does not hold a number
	...
	Product.find()
		...
		.then(numProducts => {
			...
			// control the amount of data we're retrieving from database
			// find will return us all items
			return (
				Product.find()
					// skip will allow us to skip x amount of results
					.skip((page - 1) * ITEMS_PER_PAGE)
					// limit amount of items that we're retrieving/fetching
					// we only want to get x amount of items to be displayed on a page at the same time
		...
		.then(products => {
			res.render('shop/product-list', {
				...
				path: '/products',
				currentPage: page,
				// pass an information whether there's a next page or not
